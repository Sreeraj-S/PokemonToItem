...
|notify|It's night in your game of Mafia!`);
  ...
  vote(userid, target) {
    if (!this.votingAll)
      return this.sendUser(userid, `|error|Voting is not allowed.`);
    if (this.phase !== "day")
      return this.sendUser(userid, `|error|You can only vote during the day.`);
    ...
    if (!this.enableNL && target === "novote")
      return this.sendUser(userid, `|error|No Vote is not allowed.`);
    if (target === player.id && !this.selfEnabled)
      return this.sendUser(userid, `|error|Self voting is not allowed.`);
    ...
    if (player.hammerRestriction !== null) {
      ...
      if (!player.hammerRestriction && hammering)
        return this.sendUser(userid, `|error|You cannot place the hammer vote.`);
    }
    if (player.lastVote + 2e3 >= Date.now()) {
      return this.sendUser(
        userid,
        `|error|You must wait another ${Chat.toDurationString(player.lastVote + 2e3 - Date.now()) || "1 second"} before you can change your vote.`
      );
    }
    const previousVote = player.voting;
    if (previousVote)
      this.unvote(userid, true);
    let vote = this.votes[target];
    if (!vote) {
      this.votes[target] = {
        count: 1,
        trueCount: this.getVoteValue(userid),
        lastVote: Date.now(),
        dir: "up",
        voters: [userid]
      };
    ...
    if (previousVote) {
      ...
      this.sendTimestamp(
        name === "No Vote" ? `${targetUser ? targetUser.name : userid} has abstained from voting.` : `${targetUser ? targetUser.name : userid} has voted ${name}.`
      );
    }
    player.lastVote = Date.now();
    this.hasPlurality = null;
    if (this.getHammerValue(target) <= vote.trueCount) {
      this.sendDeclare(`Hammer! ${target === "novote" ? "Nobody" : import_lib.Utils.escapeHTML(name)} was voted out!`);
      this.sendRoom(`|raw|<div class="infobox">${this.voteBox()}</div>`);
      if (target !== "novote")
        this.eliminate(target, "kill");
      this.night(true);
      return;
    }
  ...
  unvote(userid, force = false) {
    ...
    if (!player && this.dead[userid] && this.dead[userid].restless)
      player = this.dead[userid];
    if (!player?.voting)
      return this.sendUser(userid, `|error|You are not voting for anyone.`);
    if (this.voteLock && player?.voting) {
      return this.sendUser(userid, `|error|You cannot unvote because votes are locked.`);
    }
    if (player.lastVote + 2e3 >= Date.now() && !force) {
      return this.sendUser(
        userid,
        `|error|You must wait another ${Chat.toDurationString(player.lastVote + 2e3 - Date.now()) || "1 second"} before you can change your vote.`
      );
    ...
    if (!force) {
      this.sendTimestamp(
        player.voting === "novote" ? `${targetUser ? targetUser.name : userid} is no longer abstaining from voting.` : `${targetUser ? targetUser.name : userid} has unvoted ${this.playerTable[player.voting].name}.`
      );
    }
    player.voting = "";
    player.lastVote = Date.now();
  ...
  voteBoxFor(userid) {
    ...
    for (const key of Object.keys(this.playerTable).concat(this.enableNL ? ["novote"] : [])) {
      ...
      if (this.votingAll && !(this.voteLock && (isPlayer?.voting || isSpirit && this.dead[userid].voting)) && (isPlayer || isSpirit)) {
        ...
        if (vote && vote.count !== vote.trueCount)
          buf += `(${vote.trueCount})`;
  ...
  applyVoteModifier(user, target, mod) {
    ...
    if (mod === oldMod || (isNaN(mod) || mod === 1) && oldMod === void 0) {
      if (isNaN(mod) || mod === 1)
        return this.sendUser(user, `|error|${target} already has no vote modifier.`);
  ...
  eliminate(toEliminate, ability) {
    if (!(toEliminate in this.playerTable || toEliminate in this.dead))
      return;
    if (!this.started) {
      const player2 = this.playerTable[toEliminate];
      this.sendDeclare(`${player2.safeName} was kicked from the game!`);
      if (this.hostRequestedSub.includes(player2.id)) {
        this.hostRequestedSub.splice(this.hostRequestedSub.indexOf(player2.id), 1);
      }
    ...
    if (player.voting)
      this.unvote(player.id, true);
    this.sendDeclare(`${msg}! ${!this.noReveal && toID(ability) === "kill" ? `${player.safeName}'s role was ${player.getRole()}.` : ""}`);
    if (player.role && !this.noReveal && toID(ability) === "kill")
      player.revealed = player.getRole();
  ...
  revive(user, toRevive, force = false) {
    ...
    if (toRevive in this.dead) {
      ...
      this.sendDeclare(import_lib.Utils.html`${targetUser.name} has been added to the game by ${user.name}!`);
    }
    this.playerCount++;
    this.updateRoleString();
    this.updatePlayers();
  ...
  sub(player, replacement) {
    ...
    if (this.votes[oldPlayer.id]) {
      ...
      for (const p in this.playerTable) {
        if (this.playerTable[p].voting === oldPlayer.id)
          this.playerTable[p].voting = newPlayer.id;
      }
      for (const p in this.dead) {
        if (this.dead[p].restless && this.dead[p].voting === oldPlayer.id)
          this.dead[p].voting = newPlayer.id;
      }
    }
    if (this.hasPlurality === oldPlayer.id)
      this.hasPlurality = newPlayer.id;
...
|notify|You have been substituted in the mafia game for ${oldPlayer.safeName}.`);
  ...
  ideaInit(user, moduleID) {
    ...
    if (!this.IDEA.data)
      return user.sendTo(this.room, `|error|${moduleID} is not a valid IDEA.`);
    return this.ideaDistributeRoles(user);
  }
...
|notify|Pick your role in the IDEA module.`);
  ...
  sendPlayerList() {
    this.room.add(`|c:|${Math.floor(Date.now() / 1e3)}|~|**Players (${this.playerCount})**: ${Object.values(this.playerTable).map((p) => p.name).sort().join(", ")}`).update();
  }
  updatePlayers() {
    for (const p in this.playerTable) {
  ...
  setVotelock(user, setting) {
    if (!this.started)
      return user.sendTo(this.room, `The game has not started yet.`);
  ...
  clearVotes(target = "") {
    ...
    for (const player of Object.values(this.dead)) {
      if (player.restless && (!target || player.voting === target))
        player.voting = "";
    }
    this.hasPlurality = null;
  }
  ...
  mafia(query, user) {
    ...
    if (isHost) {
      ...
      for (const p in game.playerTable) {
        ...
        buf += game.voteModifiers[p] !== void 0 ? `(votes worth ${game.getVoteValue(p)})` : "";
      ...
      for (const d in game.dead) {
        ...
        if (dead.restless)
          buf += ` (is a Restless Spirit)`;
        if (game.voteModifiers[d] !== void 0)
          buf += ` (votes worth ${game.getVoteValue(d)})`;
...
const commands = {
  mafia: {
    ...
    kill(target, room, user, connection, cmd) {
      ...
      if (dead && repeat)
        return this.errorReply(`${dead.safeName} has already been ${cmd}ed.`);
      if (player || dead) {
    ...
    revealrole(target, room, user, connection, cmd) {
      ...
      for (const targetUsername of args) {
        ...
        if (!player)
          player = game.dead[toID(targetUsername)];
        if (player) {
    ...
    revive(target, room, user, connection, cmd) {
      ...
      if (didSomething)
        game.logAction(user, `added players`);
    },
    revivehelp: [
...
|selectorhtml|#mafia-votes|` + votes);
    ...
    subhost(target, room, user, connection, cmd) {
      ...
      if (targetUser.id in game.dead) {
        ...
        if (game.dead[targetUser.id].voting)
          game.unvote(targetUser.id);
